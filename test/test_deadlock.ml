...
let server_log =
  Logs.Src.create "test_deadlock_server" ~doc:"tcp deadlock tests: server"

module Server_log = (val Logs.src_log server_log : Logs.LOG)

let client_log =
  Logs.Src.create "test_deadlock_client" ~doc:"tcp deadlock tests: client"
...
module TCPIP = struct
  ...
  let get_stats _t =
    { Mirage_net.rx_pkts = 0l; rx_bytes = 0L; tx_pkts = 0l; tx_bytes = 0L }
...
let test_digest netif1 netif2 =
  ...
  let send_data () =
    ...
    TCPIP.TCPV4.create_connection
      TCPIP.(tcpv4 @@ tcpip server_stack)
      (TCPIP.client_ip, port)
    >>= function
    ...
    | Ok flow ->
        ...
        let rec read_digest chunks =
          ...
          | Ok `Eof ->
              ...
              Server_log.warn (fun f ->
                  f "!!!!!!!!!! XXXX  needed %.2fs (%.1f MB/s)" dt
                    (float (String.length data) /. dt /. (1024. ** 2.)));
              Lwt.return_unit
        in
        Lwt.pick
          [
  ...
  in
  TCPIP.TCPV4.listen
    TCPIP.(tcpv4 (tcpip client_stack))
    ~port
    (fun flow ->
      ...
      let rec consume () =
        ...
        | Error _ ->
            ...
            TCPIP.TCPV4.close flow
        | Ok `Eof ->
            TCPIP.TCPV4.write flow
            @@ Cstruct.of_string "thanks for all the fish"
            >>= fun _ -> TCPIP.TCPV4.close flow
        | Ok (`Data _data) ->
...
let run_vnetif () =
  let backend =
    Basic_backend.Make.create ~use_async_readers:true ~yield:Lwt.pause ()
  in
  TCPIP.M.NETIF.connect ~size_limit:mtu backend >>= fun c1 ->
  TCPIP.M.NETIF.connect ~size_limit:mtu backend >>= fun c2 -> test_digest c1 c2

let suite = [ ("test tcp deadlock with slow receiver", `Slow, run_vnetif) ]
