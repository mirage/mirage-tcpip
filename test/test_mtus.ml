...
let read_one flow =
  ...
  | Error e -> err_fail e
  | Ok `Eof ->
      Alcotest.fail "received EOF when we expected at least some data from read"
...
let start_client client =
  Stack.Stackv4.TCPV4.create_connection
    (Stack.Stackv4.tcpv4 client)
    (Ipaddr.V4.Prefix.address server_cidr, server_port)
  >>= function
...
let connect () =
  ...
  start_client client >>= fun flow -> Stack.Stackv4.TCPV4.close flow

let big_server_response () =
  let response = Cstruct.create 7000 in
  Cstruct.memset response 255;
  let backend = Backend.create () in
  get_stacks ~client_mtu:1500 ~server_mtu:9000 backend
  >>= fun (server, client) ->
  let f flow =
  ...
  read_all flow [] >>= fun l ->
  Alcotest.(check int)
    "received size matches sent size" (Cstruct.length response)
    (Cstruct.length (Cstruct.concat l));
...
let big_server_response_not_chunked () =
  ...
  Cstruct.memset response 255;
  let backend = Backend.create () in
  get_stacks ~client_mtu:9000 ~server_mtu:9000 backend
  >>= fun (server, client) ->
  let f flow =
    Stack.Stackv4.TCPV4.write flow response >>= function
    | Error e -> write_err_fail e
...
let suite =
  [
    ...
    ( "large client requests are chunked properly",
      `Quick,
      big_client_request_chunked );
    ( "large messages aren't unnecessarily segmented",
      `Quick,
      big_server_response_not_chunked );
