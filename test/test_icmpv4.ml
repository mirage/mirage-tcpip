...
let get_stack
    ?(backend =
      B.create ~use_async_readers:true ~yield:(fun () -> Lwt.pause ()) ()) ip =
...
let icmp_listen stack fn =
  let noop ~src:_ ~dst:_ _buf = Lwt.return_unit in
  V.listen stack.netif ~header_size (* some buffer -> (unit, error) result io *)
    (E.input stack.ethif
       ~arpv4:(Static_arp.input stack.arp)
       ~ipv6:(fun _ -> Lwt.return_unit)
       ~ipv4:
         (Ip.input stack.ip ~tcp:noop ~udp:noop ~default:(fun ~proto ->
              match proto with 1 -> fn | _ -> noop)))
  >|= fun _ -> ()
...
let short_read () =
  ...
  | Ok (icmp, _) ->
      Alcotest.fail
        (Format.asprintf
           "processed something too short to be real: %a produced %a"
           Cstruct.hexdump_pp too_short Icmpv4_packet.pp icmp)
  | Error str ->
      Printf.printf "short packet rejected successfully! msg: %s\n" str;
      Lwt.return_unit
...
let echo_request () =
  ...
  Icmpv4_packet.Marshal.into_cstruct req echo_request ~payload:request_payload
  >>=? fun () ->
  Cstruct.blit request_payload 0 echo_request Icmpv4_wire.sizeof_icmpv4
    (Cstruct.length request_payload);
  let echo_request =
    Cstruct.sub echo_request 0
      (Icmpv4_wire.sizeof_icmpv4 + Cstruct.length request_payload)
  in
  let check buf =
    ...
    | Next_hop_mtu _ | Pointer _ | Address _ | Unused ->
        Alcotest.fail
          "received an ICMP message which wasn't an echo-request or reply"
    | Id_and_seq (id, seq) ->
        Alcotest.(check int)
          "icmp response type" 0x00
          (Icmpv4_wire.ty_to_int reply.ty);
        (* expect an icmp echo reply *)
        Alcotest.(check int) "icmp echo-reply code" 0x00 reply.code;
        (* should be code 0 *)
        Alcotest.(check int) "icmp echo-reply id" id_no id;
        Alcotest.(check int) "icmp echo-reply seq" seq_no seq;
        Alcotest.(check cstruct)
          "icmp echo-reply payload" payload request_payload;
        Lwt.return_unit
  in
  Lwt.async (fun () ->
      Lwt.pick
...
let echo_silent () =
  ...
  let req =
    {
      code = 0x00;
      ty = Icmpv4_wire.Echo_request;
      subheader = Id_and_seq (0xff, 0x4341);
    }
  in
  ...
  let check buf =
    ...
    | Icmpv4_wire.Echo_reply ->
        Alcotest.fail
          "received an ICMP echo reply even though we shouldn't have"
    | msg_ty ->
        Printf.printf
          "received an unexpected ICMP message (type %s); ignoring it"
...
let write_errors () =
  ...
  let reject_all stack =
    let reject buf =
      ...
      | Ok decomposed ->
          ...
          in
          let header =
            Icmpv4_packet.Marshal.make_cstruct reply
              ~payload:decomposed.ethernet_payload
          in
          let header_and_payload =
            Cstruct.concat [ header; decomposed.ethernet_payload ]
          in
          let open Ipv4_packet in
          Icmp.write stack.icmp ~dst:decomposed.ipv4_header.src
            header_and_payload
          >|= Result.get_ok
    in
  ...
  let check_packet buf : unit Lwt.t =
    let aux buf =
      ...
      Alcotest.check Alcotest.int "ICMP message type" 0x03
        (Icmpv4_wire.ty_to_int icmp.ty);
      ...
      | _n ->
          ...
          Alcotest.(check int)
            "Payload first byte" 0x45
            (Cstruct.get_uint8 icmp_payload 0);
          Ok ()
    in
    match aux buf with Error s -> Alcotest.fail s | Ok () -> Lwt.return_unit
  in
  let check_rejection stack dst =
    let payload = Cstruct.of_string "!@#$" in
    Lwt.pick
      [
        icmp_listen stack (fun ~src:_ ~dst:_ buf ->
            check_packet buf >>= fun () -> V.disconnect stack.netif);
  ...
  inform_arp speaker listener_address (mac_of_stack listener);
  inform_arp listener speaker_address (mac_of_stack speaker);
  Lwt.pick [ reject_all listener; check_rejection speaker listener_address ]

let suite =
  [
    ("short read", `Quick, short_read);
    ("echo requests elicit an echo reply", `Quick, echo_request);
    ( "echo requests for other ips don't elicit an echo reply",
      `Quick,
      echo_silent );
