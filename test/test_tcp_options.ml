...
let test_unmarshal_zero_length () =
  ...
  Cstruct.memset bogus 1;
  (* noops *)
  Cstruct.set_uint8 bogus 0 64;
...
let test_unmarshal_stops_at_eof () =
  ...
  (* sack_ok *)
  Cstruct.set_uint8 buf 1 2;
  (* length of two *)
  Cstruct.set_uint8 buf 2 1;
  (* noop *)
  Cstruct.set_uint8 buf 3 0;
  (* eof *)
  Cstruct.set_uint8 buf 4 8;
  (* timestamp *)
  Cstruct.set_uint8 buf 5 10;
  (* timestamps are 2 4-byte times *)
  ...
  | Ok result ->
      Alcotest.(check bool)
        "SACK_ok missing" true
        (List.mem Tcp.Options.SACK_ok result);
      Alcotest.(check bool)
        "noop missing" true
        (List.mem Tcp.Options.Noop result);
      Alcotest.(check bool)
        "timestamp present" false
        (List.mem (Tcp.Options.Timestamp (ts1, ts2)) result)
...
let test_marshal_unknown () =
  ...
  let unknown = [ Tcp.Options.Unknown (64, "  ") ] in
  (* overall, length 4 *)
  Alcotest.(check int) "4 bytes" 4 (Tcp.Options.marshal buf unknown);
  (* should have written 4 bytes *)
  Cstruct.hexdump buf;
  (* option-kind *)
...
let test_marshal_into_cstruct () =
  let options =
    [
      Tcp.Options.MSS 1460; Tcp.Options.SACK_ok; Tcp.Options.Window_size_shift 2;
    ]
  in
  ...
  Alcotest.check cstruct "marshalled options are as expected" just_options
    generated_options;
...
let suite =
  [
    ...
    ( "test options marshalling when padding is needed",
      `Quick,
