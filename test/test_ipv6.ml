...
let get_stack backend address =
  ...
  Udp.connect ip >>= fun udp -> Lwt.return { backend; netif; ethif; ip; udp }

let noop ~src:_ ~dst:_ _ = Lwt.return_unit
...
let listen ?(tcp = noop) ?(udp = noop) ?(default = noop) stack =
  V.listen stack.netif ~header_size:Ethernet.Packet.sizeof_ethernet
    (E.input stack.ethif
       ...
       ~ipv6:(Ipv6.input stack.ip ~tcp ~udp ~default:(fun ~proto:_ -> default)))
...
let check_for_one_udp_packet on_received_one ~src ~dst buf =
  ...
  | Ok (_, payload) ->
      Alcotest.(check ip)
        "sender address"
        (Ipaddr.V6.of_string_exn "fc00::23")
        src;
      Alcotest.(check ip)
        "receiver address"
        (Ipaddr.V6.of_string_exn "fc00::45")
        dst;
      Alcotest.(check cstruct) "payload is correct" udp_message payload
  | Error m -> Alcotest.fail m);
  (try Lwt.wakeup_later on_received_one ()
   with _ -> () (* the first succeeds, the rest raise *));
  Lwt.return_unit
...
let send_forever sender receiver_address udp_message =
  let rec loop () =
    ...
    >|= Result.get_ok
    >>= fun () ->
    Time.sleep_ns (Duration.of_ms 50) >>= fun () -> loop ()
  in
  loop ()
...
let dad_na_is_sent () =
  ...
  let nd buf =
    Ipv6_wire.set_ipv6_version_flow buf 0x60000000l;
    (* IPv6 *)
    Ipv6_wire.set_ipv6_len buf Ipv6_wire.sizeof_ns;
    Ipaddr_cstruct.V6.write_cstruct_exn Ipaddr.V6.unspecified
      (Cstruct.shift buf 8);
    Ipaddr_cstruct.V6.write_cstruct_exn
      (Ipaddr.V6.Prefix.network_address solicited_node_prefix address)
      (Cstruct.shift buf 24);
  ...
  and is_na buf =
    ...
    Ipv6_wire.get_ipv6_version_flow buf = 0x60000000l
    (* IPv6 *)
    && Ipaddr.V6.compare
  ...
  Lwt.pick
    [
      listen stack;
      listen_raw (fun buf ->
          if is_na buf then Lwt.wakeup_later on_received_one ();
          Lwt.return_unit);
...
let dad_na_is_received () =
  ...
  let na_size =
    Ipv6_wire.sizeof_ipv6 + Ipv6_wire.sizeof_na + Ipv6_wire.sizeof_llopt
  in
  let is_ns buf =
    let icmpbuf = Cstruct.shift buf Ipv6_wire.sizeof_ipv6 in
    if
    ...
    then Some (Ipaddr_cstruct.V6.of_cstruct_exn (Cstruct.shift icmpbuf 8))
    else None
  in
  let na addr buf =
    Ipv6_wire.set_ipv6_version_flow buf 0x60000000l;
    ...
    Ipaddr_cstruct.V6.write_cstruct_exn Ipaddr.V6.link_nodes
      (Cstruct.shift buf 24);
    Ipv6_wire.set_ipv6_hlim buf 255;
    Ipv6_wire.set_ipv6_nhdr buf (Ipv6_wire.protocol_to_int `ICMP);
    let hdr, icmpbuf = Cstruct.split buf Ipv6_wire.sizeof_ipv6 in
