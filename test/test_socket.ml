...
let make_stack ~cidr =
  ...
  Stack.connect udp tcp >>= fun stack -> Lwt.return { stack; icmp; udp; tcp }

let two_connect_tcp () =
  let announce flow =
    Tcpv4_socket.read flow >>= function
    | Error _ ->
        Printf.printf "Error reading!";
        Alcotest.fail "Error reading TCP flow"
    | Ok `Eof ->
        Printf.printf "EOF!";
        Lwt.return_unit
    | Ok (`Data buf) ->
        Printf.printf "Buffer received: %s\n%!" (Cstruct.to_string buf);
        Lwt.return_unit
  in
  ...
  let teardown () =
    Stack.disconnect server.stack >>= fun () -> Stack.disconnect client.stack
  in
  ...
  Lwt.pick
    [
      ...
      ( Stack.TCPV4.create_connection client.tcp (localhost, server_port)
      >|= Result.get_ok
      >>= fun flow ->
        ...
        | Error _ ->
            teardown () >>= fun () ->
            Alcotest.fail "Error writing to socket for TCP test" );
    ]
...
let icmp_echo_request () =
  ...
  let echo_request =
    Icmpv4_packet.(
      Marshal.make_cstruct ~payload:(Cstruct.create 0)
        {
  ...
  let log_and_count buf =
    ...
    Logs.debug (fun f ->
        f "received ICMP packet number %d: %a" !received_icmp Cstruct.hexdump_pp
          buf);
    Lwt.return_unit
  in
  Lwt.pick
    [
      ...
      ( Time.sleep_ns (Duration.of_ms 500) >>= fun () ->
        Icmpv4_socket.write client.icmp ~dst:localhost echo_request
        >|= Result.get_ok
        >>= fun () -> Time.sleep_ns (Duration.of_sec 10) );
    ]
  >>= fun () ->
  ...
  Alcotest.(check int)
    "number of ICMP packets received by listener" 1 !received_icmp

let no_leak_fds_in_tcpv4 () =
  make_stack ~cidr:localhost_cidr >>= fun stack1 ->
  Stack.TCPV4.listen (Stack.tcpv4 stack1.stack) ~port:1234 (fun _flow ->
      Lwt.return_unit);
  Stack.disconnect stack1.stack >>= fun () ->
  make_stack ~cidr:localhost_cidr >>= fun stack2 ->
  Stack.TCPV4.listen (Stack.tcpv4 stack2.stack) ~port:1234 (fun _flow ->
      Lwt.return_unit);
  Stack.disconnect stack2.stack

let no_leak_fds_in_udpv4 () =
  make_stack ~cidr:localhost_cidr >>= fun stack1 ->
  Stack.UDPV4.listen (Stack.udpv4 stack1.stack) ~port:1234
    (fun ~src:_ ~dst:_ ~src_port:_ _cs -> Lwt.return_unit);
  Stack.disconnect stack1.stack >>= fun () ->
  make_stack ~cidr:localhost_cidr >>= fun stack2 ->
  Stack.UDPV4.listen (Stack.udpv4 stack2.stack) ~port:1234
    (fun ~src:_ ~dst:_ ~src_port:_ _cs -> Lwt.return_unit);
  Stack.disconnect stack2.stack
...
let make_v6_stack () =
  ...
  Stackv6.connect udp tcp >|= fun stack -> stack

let no_leak_fds_in_tcpv6 () =
  make_v6_stack () >>= fun stack1 ->
  Stackv6.TCP.listen (Stackv6.tcp stack1) ~port:1234 (fun _flow ->
      Lwt.return_unit);
  Stackv6.disconnect stack1 >>= fun () ->
  make_v6_stack () >>= fun stack2 ->
  Stackv6.TCP.listen (Stackv6.tcp stack2) ~port:1234 (fun _flow ->
      Lwt.return_unit);
  Stackv6.disconnect stack2

let no_leak_fds_in_udpv6 () =
  make_v6_stack () >>= fun stack1 ->
  Stackv6.UDP.listen (Stackv6.udp stack1) ~port:1234
    (fun ~src:_ ~dst:_ ~src_port:_ _cs -> Lwt.return_unit);
  Stackv6.disconnect stack1 >>= fun () ->
  make_v6_stack () >>= fun stack2 ->
  Stackv6.UDP.listen (Stackv6.udp stack2) ~port:1234
    (fun ~src:_ ~dst:_ ~src_port:_ _cs -> Lwt.return_unit);
  Stackv6.disconnect stack2
...
let make_v4v6_stack ipv4_only ipv6_only ipv4 ipv6 =
  ...
  Stackv4v6.connect udp tcp >|= fun stack -> stack
...
let no_leak_fds_in_tcpv4v6 () =
  ...
  Stackv4v6.TCP.listen (Stackv4v6.tcp stack1) ~port:1234 (fun _flow ->
      Lwt.return_unit);
  Stackv4v6.disconnect stack1 >>= fun () ->
  make_v4v6_stack false false ip4_any None >>= fun stack2 ->
  Stackv4v6.TCP.listen (Stackv4v6.tcp stack2) ~port:1234 (fun _flow ->
      Lwt.return_unit);
  Stackv4v6.disconnect stack2

let no_leak_fds_in_udpv4v6 () =
  make_v4v6_stack false false ip4_any None >>= fun stack1 ->
  Stackv4v6.UDP.listen (Stackv4v6.udp stack1) ~port:1234
    (fun ~src:_ ~dst:_ ~src_port:_ _cs -> Lwt.return_unit);
  Stackv4v6.disconnect stack1 >>= fun () ->
  make_v4v6_stack false false ip4_any None >>= fun stack2 ->
  Stackv4v6.UDP.listen (Stackv4v6.udp stack2) ~port:1234
    (fun ~src:_ ~dst:_ ~src_port:_ _cs -> Lwt.return_unit);
  Stackv4v6.disconnect stack2
...
let suite =
  [
    ...
    ( "file descriptors are not leaked in tcpv4v6 (any)",
      `Quick,
      no_leak_fds_in_tcpv4v6 );
    ( "file descriptors are not leaked in udpv4v6 (any)",
      `Quick,
      no_leak_fds_in_udpv4v6 );
    ( "file descriptors are not leaked in tcpv4v6 (v4)",
      `Quick,
      no_leak_fds_in_tcpv4v6_2 );
    ( "file descriptors are not leaked in udpv4v6 (v4)",
      `Quick,
      no_leak_fds_in_udpv4v6_2 );
    ( "file descriptors are not leaked in tcpv4v6 (v4v6)",
      `Quick,
      no_leak_fds_in_tcpv4v6_3 );
    ( "file descriptors are not leaked in udpv4v6 (v4v6)",
      `Quick,
      no_leak_fds_in_udpv4v6_3 );
    ( "file descriptors are not leaked in tcpv4v6 (v4 only)",
      `Quick,
      no_leak_fds_in_tcpv4v6_4 );
    ( "file descriptors are not leaked in udpv4v6 (v4 only)",
      `Quick,
      no_leak_fds_in_udpv4v6_4 );
    ( "file descriptors are not leaked in tcpv4v6 (v6 only)",
      `Quick,
      no_leak_fds_in_tcpv4v6_5 );
    ( "file descriptors are not leaked in udpv4v6 (v6 only)",
      `Quick,
      no_leak_fds_in_udpv4v6_5 );
  ]
