...
let fails msg f args =
  match f args with Ok _ -> Alcotest.fail msg | Error _ -> ()
...
let write () =
  ...
  Udp.write ~src_port:1212 ~dst_port:21 ~dst stack.udp
    (Cstruct.of_string "MGET *")
  >|= Result.get_ok
...
let unmarshal_regression () =
  ...
  Alcotest.(check (result reject pass))
    "correctly return error for bad packet" (Error "parse failed")
    (Udp_packet.Unmarshal.of_cstruct i);
  Lwt.return_unit
...
let marshal_marshal () =
  ...
  let pseudoheader =
    Ipv4_packet.Marshal.pseudoheader ~src ~dst ~proto:`UDP
      (Cstruct.length buffer + Cstruct.length payload)
  in
  Udp_packet.Marshal.into_cstruct ~pseudoheader ~payload udp
    (Cstruct.shift buffer 1)
  |> Alcotest.check error_str "Buffer too short"
       (Error "Not enough space for a UDP header");
  Udp_packet.Marshal.into_cstruct ~pseudoheader ~payload udp buffer
  |> Alcotest.(check (result unit string))
       "Buffer big enough for header" (Ok ());
  Udp_packet.Unmarshal.of_cstruct (Cstruct.concat [ buffer; payload ])
  |> Alcotest.(check (result (pair udp_packet cstruct) string))
       "Save and reload"
       (Ok (udp, payload));
  Lwt.return_unit
