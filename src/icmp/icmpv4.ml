  ...
  let input t ~src ~dst:_ buf =
    ...
    | Ok (message, payload) -> (
        ...
        | Echo_request, Id_and_seq (id, seq) ->
            Log.debug (fun f ->
                f "ICMP echo-request received: %a (payload %a)" Icmpv4_packet.pp
                  message Cstruct.hexdump_pp payload);
            if t.echo_reply then (
              let icmp =
                {
                  code = 0x00;
              ...
              | Error (`Ip e) ->
                  Log.warn (fun f ->
                      f "Unable to send ICMP echo-reply: %a" IP.pp_error e);
                  ())
            else Lwt.return_unit
        | ty, _ ->
            Log.info (fun f ->
                f "ICMP unknown ty %s from %a" (ty_to_string ty) Ipaddr.V4.pp
                  src);
            Lwt.return_unit)
end
