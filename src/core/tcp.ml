...
module Keepalive = struct
  type t = { after : Duration.t; interval : Duration.t; probes : int }
end
...
module type S = sig
  ...
  val writev_nodelay :
    flow -> Cstruct.t list -> (unit, write_error) result Lwt.t

  val create_connection :
    ?keepalive:Keepalive.t -> t -> ipaddr * int -> (flow, error) result Lwt.t
