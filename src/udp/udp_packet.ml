...
let pp fmt t = Format.fprintf fmt "UDP port %d -> %d" t.src_port t.dst_port
...
module Unmarshal = struct
  ...
  let of_cstruct buf =
    ...
    let check_header_length () =
      if Cstruct.length buf < sizeof_udp then Error "UDP header too short"
      else Ok ()
    in
    let check_payload_length length_from_header length_of_buffer =
      if length_from_header < sizeof_udp then
        ...
        if payload_len > length_of_buffer - sizeof_udp then
          Error
            (Printf.sprintf
               "UDP header claimed a payload longer than the supplied buffer: \
                %d vs %d."
    ...
    let* payload_len =
      check_payload_length total_length_from_header (Cstruct.length buf)
    in
...
module Marshal = struct
  ...
  let unsafe_fill ~pseudoheader ~payload { src_port; dst_port } udp_buf len =
    ...
    let csum =
      Tcpip_checksum.ones_complement_list [ pseudoheader; udp_buf; payload ]
    in
  ...
  let into_cstruct ~pseudoheader ~payload t udp_buf =
    ...
    in
    Result.bind (check_header_len ()) (fun () ->
