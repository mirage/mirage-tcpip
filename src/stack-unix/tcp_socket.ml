...
let rec write fd buf =
  Lwt.catch
    (fun () ->
      Lwt_cstruct.write fd buf >>= function
...
let writev fd bufs =
  Lwt_list.fold_left_s
    (fun res buf ->
      match res with Error _ as e -> return e | Ok () -> write fd buf)
    (Ok ()) bufs

(* TODO make nodelay a flow option *)
let write_nodelay fd buf = write fd buf

(* TODO make nodelay a flow option *)
let writev_nodelay fd bufs = writev fd bufs
