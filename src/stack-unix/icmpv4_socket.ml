...
let pp_sockaddr fmt sa =
  ...
  | ADDR_INET (ip, port) ->
      Format.fprintf fmt "%s, %d" (Unix.string_of_inet_addr ip) port
...
let sendto' fd buf flags dst =
  ...
  else Lwt_cstruct.sendto fd buf flags dst
...
let recvfrom' fd buf flags =
  ...
  else Lwt_cstruct.recvfrom fd buf flags

let write _t ?src:_ ~dst ?ttl:_ttl buf =
  let open Lwt_unix in
  let flags = [] in
  let ipproto_icmp = 1 in
  (* according to BSD /etc/protocols *)
  let port = 0 in
  (* port isn't meaningful in this context *)
  ...
  Lwt.catch
    (fun () ->
      ...
      if sent <> Cstruct.length buf then
        Log.debug (fun f ->
            f "short write: %d received vs %d expected" sent
              (Cstruct.length buf));
...
let input t ~src ~dst:_ buf =
  ...
  | Ok (icmp, payload) -> (
      ...
      | Icmpv4_wire.Echo_request, Id_and_seq (id, seq) ->
          ...
          write t ~dst:src (Marshal.make_cstruct response ~payload) >>= fun _ ->
          Lwt.return_unit
      | _, _ -> Lwt.return_unit)
...
let listen t addr fn =
  ...
  let sa =
    Lwt_unix.ADDR_INET
      (Unix.inet_addr_of_string (Ipaddr.V4.to_string addr), port)
  in
  ...
  let rec loop () =
    ...
    Ipv4_wire.set_ipv4_len receive_buffer
      (min len (Cstruct.length receive_buffer));
