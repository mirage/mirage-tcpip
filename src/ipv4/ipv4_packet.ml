...
let pp fmt t =
  Format.fprintf fmt
    "IPv4 packet %a -> %a: id %04x, off %d proto %d, ttl %d, options %a"
    Ipaddr.V4.pp t.src Ipaddr.V4.pp t.dst t.id t.off t.proto t.ttl
    Cstruct.hexdump_pp t.options

let equal { src; dst; id; off; ttl; proto; options } q =
  src = q.src && dst = q.dst && id = q.id && off = q.off && ttl = q.ttl
  && proto = q.proto
...
module Marshal = struct
  ...
  let protocol_to_int = function `ICMP -> 1 | `TCP -> 6 | `UDP -> 17
  ...
  let unsafe_fill ~payload_len t buf =
    let nearest_4 n = match n mod 4 with 0 -> n | k -> 4 - k + n in
    ...
    let checksum =
      Tcpip_checksum.ones_complement @@ Cstruct.sub buf 0 (20 + options_len)
    in
  ...
  let into_cstruct ~payload_len t buf =
    ...
    else Ok (unsafe_fill ~payload_len t buf)
...
module Unmarshal = struct
  ...
  let header_of_cstruct buf =
    ...
    let check_version buf =
      ...
      | n ->
          Error
            (Printf.sprintf
               "IPv4 presented with a packet that claims a different IP \
                version: %x"
               n)
    in
    let size_check buf =
      if Cstruct.length buf < sizeof_ipv4 then
        Error "buffer sent to IPv4 parser had size < 20"
      else Ok ()
    in
    let get_header_length buf =
      ...
      if get_ipv4_len buf < sizeof_ipv4 then
        ...
        Error
          (Printf.sprintf
             "IPv4 packet w/length %d claimed to have header of size %d"
             (Cstruct.length buf) hlen)
      else Ok hlen
    in
    let parse buf options_end =
      ...
      let options =
        if options_end > sizeof_ipv4 then
          Cstruct.sub buf sizeof_ipv4 (options_end - sizeof_ipv4)
  ...
  let of_cstruct buf =
    ...
    let parse buf options_end =
      ...
      if payload_available < payload_len then
        Error
          (Printf.sprintf
             "Payload buffer (%d bytes) too small to contain payload (of size \
              %d from header)"
             payload_available payload_len)
      else
  ...
  let verify_transport_checksum ~proto ~ipv4_header ~transport_packet =
    ...
    let check ~proto ipv4_header len =
      try
        let ph =
          Marshal.pseudoheader ~src:ipv4_header.src ~dst:ipv4_header.dst ~proto
            len
        in
        let calculated_checksum =
          Tcpip_checksum.ones_complement_list [ ph; transport_packet ]
        in
